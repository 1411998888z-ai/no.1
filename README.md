...
|---|---|
| **Gemini運用（推奨）** | **完全無料** |
| Claude運用 | 約120円（投稿文の質を最大化したい場合） |

GitHub Actions / Cloudflare Workers / LINE Messaging API / Threads API はすべて無料枠内で運用可能。
切替は GitHub Variables の `LLM_PROVIDER` を `gemini` または `claude` に設定するだけ。
...
cp .env.example .env  # 値を埋める（LLM_PROVIDER=gemini で無料運用）
