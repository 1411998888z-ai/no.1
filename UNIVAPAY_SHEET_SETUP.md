...
> **カード名義・氏名・メール等について**：これらは決済(charge)データに含まれず、取引トークン側にあります。
> `UNIVAPAY_JWT` と `UNIVAPAY_SECRET` を設定すると、Webhook 受信時にトークンAPIへ問い合わせて
> 「カード名義」「氏名（漢字）」「フリガナ」「電話番号」「メールアドレス」列を自動で埋めます。
> 氏名・フリガナ・電話はトークンの metadata（`univapay-name` / `univapay-name-furigana` /
> `univapay-phone-number`）から取得します。未設定ならこれらの列は空欄になり、他は通常どおり動きます。
